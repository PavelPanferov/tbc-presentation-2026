          ...
          <div className="flex gap-1.5 items-center">
            {ALL_SLIDES.map((_, idx) => (
              <button key={idx} onClick={() => goTo(idx)}
                className={`h-2 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-8 bg-wd-purple' : 'w-2 bg-gray-300 hover:bg-gray-400'}`} />
            ))}
          </div>

          {/* Center: metadata */}
          <div className="flex items-center gap-12 text-xs text-wd-gray">
            <div><span className="text-wd-muted">Презентация:</span> <span className="font-semibold text-wd-dark">TBC Bank</span></div>
            <div><span className="text-wd-muted">Тема:</span> <span className="font-semibold text-wd-dark">AI Marketing Platform</span></div>
            <div><span className="text-wd-muted">Агентство:</span> <span className="font-semibold text-wd-dark">We Digital</span></div>
            <div><span className="text-wd-muted">Год:</span> <span className="font-semibold text-wd-dark">2026</span></div>
          </div>

          {/* Right: arrows */}
          <div className="flex gap-2">
            <button onClick={handlePrev} disabled={currentSlide === 0}
              className="p-2 border border-gray-200 hover:border-wd-purple hover:text-wd-purple text-gray-400 disabled:opacity-20 rounded-lg transition-all">
              <ChevronLeft size={18} />
            </button>
